...
public:
	...
	//enum MoveType : int
	//{
	//	MOVETYPE_NONE = 0,         // No movement type (usually used for non-player entities)
	//	MOVETYPE_ISOMETRIC,        // Movement on the XY plane, no gravity
	//	MOVETYPE_WALK,             // Normal walking movement
	//	MOVETYPE_STEP,             // Movement when going up stairs
	//	MOVETYPE_FLY,              // Free-flying movement (noclip)
	//	MOVETYPE_FLYGRAVITY,       // Free-flying movement with gravity (noclip)
	//	MOVETYPE_VPHYSICS,         // Physics-based movement
	//	MOVETYPE_PUSH,             // Movement when being pushed by other entities
	//	MOVETYPE_NOCLIP,           // No-clip movement (ignores collisions)
	//	MOVETYPE_LADDER,           // Movement on a ladder
	//	MOVETYPE_OBSERVER,         // Observer movement (spectating)
	//	MOVETYPE_CUSTOM,           // Custom movement, defined by the entity
...
public: // netvars
	...
	NETVAR(FlashDur, "CBasePlayer->m_flFlashDuration", float)
	NETVAR(Velocity, "CBasePlayer->m_vecVelocity[0]", CVector)
	//NETVAR(GetMoveType, "CBaseEntity->m_nRenderMode", int)
